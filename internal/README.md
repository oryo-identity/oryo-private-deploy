...
| `scripts/provision.sh` | The mutating counterpart to `customer/scripts/setup.sh` — creates the AWS prerequisites (bucket, IAM policy + IRSA role, subnet tags, NodePool arm64). Oryo uses it to stand up the sandbox or to provision on a customer's behalf. Each step maps 1:1 to a section of `customer/docs/prereqs.md`, with the rationale inline. |
