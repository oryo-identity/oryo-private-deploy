...
# GHCR pull secret: needed to pull Oryo's images; not generated by
# --bootstrap-secrets (it holds the Oryo-issued token, docs/runbook.md §2)
kubectl -n "$NAMESPACE" get secret ghcr-pull >/dev/null 2>&1 && ok "ghcr-pull exists" \
  || bad "secret ghcr-pull missing in namespace $NAMESPACE. Create it from your Oryo-issued GHCR token: kubectl -n $NAMESPACE create secret docker-registry ghcr-pull --docker-server=ghcr.io --docker-username=<account> --docker-password=<token>"
