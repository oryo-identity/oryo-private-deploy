...
Oryo's container images live in Oryo's prod ECR (`831622638566.dkr.ecr.us-east-1.amazonaws.com`). For a customer cluster to pull them, Oryo adds the customer's account ID to each repo's policy. One-time per customer account.

You send Oryo your AWS account ID + region; Oryo grants pull access to `api`, `dashboard`, `gateway`, `workers`, `db-init`.
