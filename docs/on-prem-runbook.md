...
auto-selects amd64 on amd64 nodes. Confirm:
...
### 3c. Ingress + TLS (replaces ALB / ACM / Route 53)  [validated]

The chart ships AWS ALB ingress annotations — harmless, nginx ignores them. On-prem you provide an
ingress controller, a TLS cert, and name resolution.
...
**TLS cert** — an internal CA cert in prod; self-signed for the lab:
```bash
...
> ignores them. TLS cert + internal DNS to actually *reach* the services is the remaining work
> (§3c). The pods themselves install and run with the above.
...
- Point `app` / `api` / `gateway.<DOMAIN>` at the ingress's IP via **internal DNS**.
- Make sure the endpoints trust the platform's TLS cert (push the internal CA via MDM/AD).
- Install sensors per the dashboard's **Settings → Installation** (CA download + install one-liner;
  roll out via Intune/MDM).
